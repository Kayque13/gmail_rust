...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut imap_session = client.login(username, password).map_err(|e| format!("Erro de login: {}", e.0))?;
    imap_session.select("INBOX")?;
    let mailbox = imap_session.examine("INBOX")?;
    let total_messages = mailbox.exists;
    let start = if total_messages > 50 { total_messages - 49 } else { 1 };
    let range = format!("{}:{}", start, total_messages);
    let messages = imap_session.fetch(&range, "RFC822")?;
    println!("{}", "===Lista de Emails===".bold().cyan());
    for (i, message) in messages.iter().enumerate() {
        if let Some(body) = message.body() {
            let parsed = parse_mail(body)?;
            let from = parsed.headers.iter().find(|h| h.get_key().to_lowercase() == "from").map(|h| h.get_value()).unwrap_or("Desconhecido".to_string());
            let subject = parsed.headers.iter().find(|h| h.get_key().to_lowercase() == "subject").map(|h| h.get_value()).unwrap_or("Sem Assunto".to_string());
            let date = parsed.headers.iter().find(|h| h.get_key().to_lowercase() == "date").map(|h| h.get_value()).unwrap_or("Desconhecido".to_string());
...
fn extract_text_body(parsed: &mailparse::ParsedMail) -> String {
    if parsed.ctype.mimetype == "text/plain" {
        return parsed.get_body().unwrap_or("Erro ao decodificar corpo".to_string());
    }
    for part in &parsed.subparts {
        if part.ctype.mimetype == "text/plain" {
            return part.get_body().unwrap_or("Erro ao decodificar corpo".to_string());
        }
    }
